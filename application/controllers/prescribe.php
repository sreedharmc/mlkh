...
class Prescribe extends Secure_area
{
	...
	{
		$data=array();
		$mode = $this->prescribe_lib->get_mode();
		$item_id_or_number_or_receipt = $this->input->post("item");
		$quantity = $mode=="sale" ? 1:-1;

		if($this->prescribe_lib->is_valid_receipt($item_id_or_number_or_receipt) && $mode=='return')
		{
			$this->prescribe_lib->return_entire_invoice($item_id_or_number_or_receipt);
		}
		elseif(!$this->prescribe_lib->add_item($item_id_or_number_or_receipt,$quantity))
		{
			$data['error']=$this->lang->line('invoices_unable_to_add_item');
		}
		
		if($this->prescribe_lib->out_of_stock($item_id_or_number_or_receipt))
		{
	...
	function edit_item($line)
	{
		...
		{
			$this->prescribe_lib->edit_item($line,$description,$serialnumber,$quantity,$discount,$price);
		}
		else
		{
		...
		if($this->prescribe_lib->out_of_stock($this->prescribe_lib->get_item_id($line)))
		{
			$data['warning'] = $this->lang->line('invoices_quantity_less_than_zero');
		}
	...
	{
		$this->prescribe_lib->delete_item($item_number);
		$this->_reload();
	}

	function remove_customer()
	{
		$this->prescribe_lib->delete_customer();
		$this->_reload();
	}

	function complete()
	{
		$data['cart']=$this->prescribe_lib->get_cart();
		$data['subtotal']=$this->prescribe_lib->get_subtotal();
		$data['total']=$this->prescribe_lib->get_total();
		...
		$this->load->view("prescribe/receipt",$data);
		$this->prescribe_lib->clear_all();
	}

	function receipt($invoice_id)
	{
		$invoice_info = $this->Invoice->get_invoice($invoice_id)->row_array();
		$this->prescribe_lib->copy_entire_invoice($invoice_id);
		$data['cart']=$this->prescribe_lib->get_cart();
		$data['subtotal']=$this->prescribe_lib->get_subtotal();
		$data['total']=$this->prescribe_lib->get_total();
	...
	function _reload($data=array())
	{
		...
		$data['subtotal']=$this->prescribe_lib->get_subtotal();
		$data['taxes']=$this->prescribe_lib->get_taxes();
		$data['total']=$this->prescribe_lib->get_total();
		$data['items_module_allowed'] = $this->Employee->has_permission('items', $person_info->person_id);
		//Alain Multiple Payments
		$data['payments_total']=$this->prescribe_lib->get_payments_total();
		$data['amount_due']=$this->prescribe_lib->get_amount_due();
		...
		$this->load->view("prescribe/register",$data);
	}
